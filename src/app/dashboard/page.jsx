...
async function loadOrdersMonth() {
	const { data } = await axios.get("http://localhost:3000/api/orders/summary");
	return data;
}

async function loadReservationsMonth() {
	const { data } = await axios.get(
		"http://localhost:3000/api/reservations/summary"
	);
	return data;
}
				...
				<div className="grid gap-6 mb-8 md:grid-cols-2 lg:grid-cols-3">
					...
					<Card className="rounded-xl">
						...
						<CardContent>
							...
							<p className="text-xs text-muted-foreground">
								{ordersMonth.percentageChange} comparado con el mes anterior
							</p>
						</CardContent>
					</Card>
					<Card className="rounded-xl">
					...
					<CardContent>
						<Table className="w-full">
							...
							<TableBody>
								{orders.map((order) => (
									<TableRow
										// href={`/orders/${order.comanda_id}`}
										className="w-full cursor-pointer"
										key={order.comanda_id}
									...
									>
										...
										<TableCell className="text-center w-1/5">
											{order.modelo_patente}
										</TableCell>
										<TableCell className="text-center w-1/5">
											{new Date(order.fecha_instalacion).toLocaleDateString(
												"es-AR",
												{
													day: "2-digit",
													month: "2-digit",
